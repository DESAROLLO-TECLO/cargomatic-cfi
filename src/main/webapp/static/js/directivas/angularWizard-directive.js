...
//            })
    ...
    return {
        ...
        controller: ["$scope", "$element", "$log", "WizardHandler", "$q", "$timeout", function($scope, $element, $log, WizardHandler, $q, $timeout) {
            ...
            function personalPorPasos(){//pendiente para caso de querer validar desde el icono de pasos del wizard
            	if(angular.isDefined($scope.functionPersonal))
            		 $scope.functionPersonal && $scope.functionPersonal()
            }
            ...
            },this.addStepPosition = function(a,position){
            	$scope.steps.splice(position,0,a),$scope.getEnabledSteps()[0] === a && $scope.goTo($scope.getEnabledSteps()[0])
            }
            , this.removeStep = function(a) {
                var b = $scope.steps.indexOf(a);
                b > -1 && $scope.steps.splice(b, 1)
